...
- 输入：
  ...
  - `required_files`
- 输出：
  - `checked_files`：每个必需文件或目录的检查结果，包含存在状态、评分、命中章节和缺失章节。
  - `total_score`：所有文件的工程化评分总分（满分90分）。
  - `max_total_score`：满分总分（90分）。
  - `missing_files`：所有缺失的必需文件或目录。
  - `status_hint`：全部存在且评分达标时为 `PASS`，全部存在但评分不达标时为 `WARNING`，存在缺失项时为 `BLOCKED`。
...
| 状态 | 含义 | 下一步 |
| --- | --- | --- |
| `PASS` | 必需文件齐全且工程化评分达标（≥80%），并且测试通过。 | 允许提交，或继续后续任务。 |
| `WARNING` | 必需文件齐全但工程化评分不达标（<80%），或测试出现可恢复超时。 | 记录风险，补充文件缺失章节或重新运行检查。 |
...
| 条件 | 状态 | 下一步 |
| --- | --- | --- |
| `required_files` 全部存在，且工程化评分达标（≥80%），且 `test_command` 通过。 | `PASS` | 可以提交或进入下一项任务。 |
| 必需文件全部存在，但工程化评分不达标（<80%）。 | `WARNING` | 补充文件缺失章节，提高工程化评分，然后重新运行 `checkFiles()`。 |
